<cfoutput>
    ...
    <cfset local.missingData=EntityLoad("missingDataORM",{userId="#session.userId#",fileName="#session.fileName#"})>
    ...
    <cfloop array=#local.contactData# index='local.contact'>
        ...
        <cfset local.Result=''>
        <cfset local.resultData=entityLoad("resultORM",{contactId=local.contact,fileName="#session.fileName#"})>
    ...
    <cfspreadsheet action="write" filename="#excelFilePath#" query="local.excelQry" sheetname="results">
